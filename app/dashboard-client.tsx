...
export default function DashboardClient() {
  ...
  const handleCheckpointClick = (id: number) => {
    ...
    // Resolver ID único do livro correspondente
    let bookId = `book-${id < 10 ? '0' + id : id}`;
    if (activeTripId === 'great-commission') {
      bookId = `great-commission-0${id}`;
    } else if (activeTripId === 'daily-life') {
      bookId = `book-${id + 16}`;
    }

    // Abrir a nova rota dedicada de leitura isolada em uma NOVA ABA
